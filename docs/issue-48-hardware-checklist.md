...
- [x] PLAY/PAUSE toggles the deck (realtime and playback modes).
- [x] Channel faders ride volume smoothly, full 14-bit (no coarse steps).
- [x] Crossfader sweeps A↔B; HEADPHONES MIX blends cue↔master.
- [x] EQ HI/MID/LOW per deck; TRIM sets manual gain and drops auto-trim.
- [x] SMART CFX rides the Color FX amount; SHIFT + CFX sweeps the style
      cursor; release restores FX.
- [x] HOT CUE pads: realtime = net selection toggles; playback = set/jump
      cues; SHIFT + pad clears a cue (shift pad layer).
- [x] PAD FX pads select/deselect effects; SAMPLER pads drive the freeze
      slots; SHIFT + SAMPLER clears a slot.
- [x] Browse rotary scrolls the explorer (direction correct, fast turns
      multi-step); rotary press cycles tabs; LOAD loads onto each deck.
- [x] Jog: paused fine-seek, playing phase-nudge, SHIFT+jog scrub
...
- [x] Tempo sliders varispeed a playback deck (low = slow end); LOOP
      IN/OUT, 4 BEAT/EXIT toggle, CUE/LOOP CALL halve/double.
- [x] BEAT FX ON/OFF toggles recording; channel CUE toggles PFL; transport
      CUE preps/stops.

## LEDs (now painted from the store by the shell)

- [x] On bind, the LEDs match the app state (target pads, FX pad, filled
      loop slots, channel/transport CUE) without touching anything.
- [x] Switching pad mode on the device repaints the freshly-selected bank
      (the device clears pad LEDs on a switch; the shell repaints).
- [x] The mode selector LEDs follow the active bank: press PAD FX1 / BEAT
...
- [x] Arm KEYBOARD, then unplug/replug the controller: on rebind the HOT
...
- [x] The net's bright/dim distinction reads on the pads; if `0x20` is not
...
- [x] LED changes track app actions live: select an FX → its pad lights;
...
- [x] Arm deck A (pad-mode selector or the panel button); the worker drops
...
- [x] Chord-follow (default): hold pad 1 (the tonic triad) — the deck's
      harmony audibly moves to it **within ~one beat**; hold pad 5 against
      it — the change lands as a chord change, not a glitch.
- [x] Snap-to-scale: with key C major, every pad is consonant; switch the
...
- [x] Onset mode: tap a pad — the attack lands **on the next beat** (audibly
...
- [x] Releases: letting go of all pads returns the deck to free generation
      within a few chunks (masked, not silence).
- [x] External keyboard: with deck A armed, keys steer it (snapped); with
      both decks armed, keys steer both; disarmed decks ignore it.
- [x] Play/stop/model-switch mid-hold: steering resets cleanly (no stale
      notes over the fresh stream), the panel's held readout clears, and
      the disarm→1 s chunk path restores after disarming.
- [x] MCP `set_notes` still steers by ear (rerun the issue-46 checklist's
...
- [x] With deck A streaming (or freshly stopped), press PLAY on the other
...
- [x] MCP `set_hot_cue`/`clear_hot_cue` light/clear the on-screen pads
      immediately; a pad set on-screen right after is kept.

## Regression sweeps

- [x] `just check` green; the app boots with sidecars and no MIDI-related
      errors in the console.
- [x] DDJ-400 pass: repeat the control-surface + LED sections on the
      DDJ-400 (shared byte scheme; its own position SysEx).
- [x] A long set (~15 min) with hardware in use: no MIDI dropouts, no
