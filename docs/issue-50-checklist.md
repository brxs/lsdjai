...
- [x] `just tauri-dev`, app open; deck A playing a drum-heavy style (e.g.
      "four on the floor techno" plus something pitched, e.g. "warm dub
      chords").
- [x] Open deck A's performance door (the Config rail on the prompt pad):
...
- [x] Flip **No drums** on for the playing deck: the kit audibly
...
- [x] Flip it back off: the model's own drum choice returns.
- [x] It works with **MIDI steering off** — the toggle never arms the deck
      (the steer switch stays off, the chunk cadence unchanged).

## Drums adherence (cfg_drums, issue #50)

- [x] The **Drums adherence** slider is **always shown** (independent of the
      No-drums toggle), range **0-5**, defaulting to **4** (the reference's
      `DEFAULT_CFG_DRUMS`).
- [x] With steering on, lowering adherence toward 0 audibly weakens the
...
- [x] With steering **off** (auto), the adherence value still reaches the
...
- [x] Arm a deck (MIDI steering on), set Note mode to **On-grid onset**, and
      hold a pad/key: the note attacks once and then **rings/sustains** — no
      machine-gun re-attack at the ~5 Hz chunk rate.
- [x] **Chord follow** is unchanged: held pads still re-voice per chunk (the
...
- [x] General playback sounds musical, not degraded — with the looser prompt
...
- [x] Note steering (issue #48) binds *more* tightly to held notes than before
      (`cfg_notes` 2.4 vs old 1.0) — held chords should track more strongly.
- [x] Non-held pitches are now MASKED, not off (matching the reference): a held
...
- [x] MCP `set_drums` (suppress/auto) moves the drawer toggle; the toggle is
